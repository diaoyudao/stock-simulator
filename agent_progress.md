...
### Done
- 市场状态显示：页面头部显示交易状态 + 交易时间段
  - 后端 `GET /trade/market-status` 接口：返回 is_trading_time、status、sessions
  - 前端 `useTradingTime()` hook 改用后端数据（降级到前端计算）
- 法定节假日处理：后端硬编码2026年节假日+调休数据
  - 33天节假日（元旦/春节/清明/劳动节/端午/中秋/国庆）
  - 5天调休上班日（周末但需上班）
  - 买卖交易、交易时间判断均受节假日影响
  - timor.tech API 有 Cloudflare 防护无法服务端调用，改用本地数据
